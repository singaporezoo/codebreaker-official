from __future__ import print_function
import pickle
import io
import json
import os
from time import sleep
import boto3 # Amazon S3 client library
from googleapiclient.discovery import build # Google drive API service
from googleapiclient.http import MediaIoBaseDownload # Google drive API downloader
s3 = boto3.resource('s3')
BUCKET_NAME = 'codebreaker-testdata' # Name of S3 bucket to upload testdata
rootId = '1YsKMAesnXOfgwMZ_q33E-Mvq2ZQShhVy' # Google drive index of codebreaker testdata
lambda_client = boto3.client('lambda')

def lambda_handler(event, context):
    folderName = event["folderName"] # Name of problem

    print("Updating {0}".format(folderName))
    
    creds = None
    with open('token.pickle', 'rb') as token:
        creds = pickle.load(token)

    # Building google drive client
    service = build('drive', 'v3', credentials=creds)
    service_v2 = build('drive', 'v2', credentials=creds)

    # Querying for google drive folder with name as the problem name and is located inside the codebreaker-testdata folder
    results = service.files().list(
        q="name = '{0}' and '{1}' in parents".format(folderName,rootId)
    ).execute()

    items = results.get('files', [])

    if len(items) != 1:
        return {
            'statusCode': 300, #Database transaction failed
            'errorMessage': 'Testdata folder not found'
        }

    folderId = items[0]['id']

    while True:
        results = service.files().list(
            q="parents = '{0}'".format(folderId)
        ).execute()
        items = results.get('files', [])

        if len(items) == 0: # Folder is empty
            break

        items.sort(key = lambda x:x['name'])

        testcase = items[0]
        filename=str(testcase['name'])
        id=testcase['id']

        ind = filename.split('.')
        if len(ind) != 2:
            return {
                'statusCode': 300,
                'errorMessage': 'File Name {0} has invalid syntex'.format(filename)
            }

        if ind[1] != 'in' and ind[1] != 'out':
            return {
                'statusCode': 300,
                'errorMessage': 'File Name {0} has invalid testcase number'.format(filename)
            }

        try: 
            l = int(ind[0])
        except ValueError:
            return {
                'statusCode': 300,
                'errorMessage': 'File Name {0} has invalid index'.format(filename)
            }

        try:
            request = service.files().get_media(fileId=id)
            fh = io.BytesIO()
            downloader = MediaIoBaseDownload(fd=fh, request=request)
            # Downloading file from google drive
            done = False
            while not done:
                status,done = downloader.next_chunk()
        except:
            return {
                'statusCode': 301, #Database transaction failed
                'errorMessage': 'Lambda is already running'
            }

        filename = str(int(ind[0])) + '.' + ind[1]
        fh.seek(0)
        lambda_path='/tmp/{0}'.format(filename)

        with open(lambda_path,'wb') as f:
            f.write(fh.read())
            f.close()

        # Pushing file onto AWS S3
        s3path='{0}/{1}'.format(folderName,filename)
        s3.meta.client.upload_file(lambda_path,BUCKET_NAME,s3path) 

        # Clearing download to ensure that lambda does not run into memory issues
        os.remove(lambda_path)
        initlen = len(items)

        try:
            # If possible, delete file permentantly
            service.files().delete(fileId=id).execute()
            print("Deleted file {0}".format(filename))
        except:
            return{
                'statusCode':302,
                'errorMessage':'Permissions Error'
            }


    # Removing folder from google drive
    service_v2.parents().delete(fileId=folderId,parentId=rootId).execute()

    lambda_input = {"problemName":folderName}
    lambda_client.invoke(FunctionName = "arn:aws:lambda:ap-southeast-1:354145626860:function:codebreaker-update-testcaseCount", InvocationType='Event', Payload=json.dumps(lambda_input))

    return {
        'statusCode': 200
    }
